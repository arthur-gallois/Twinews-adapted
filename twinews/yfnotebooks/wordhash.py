...
class WordHash(object):
    ...
    def __init__(self, words, n_gram=3, marks='#', load_from_file=False, load_file =None, dump_to_file=False, dump_file=None):
        ...
        self.words = list(map(lambda x: marks+x.lower()+marks, list(set(self.ori_words))))
        ...
        for one_sen in sentences:
            ...
            for one_word in one_sen:
                ...
                for splited in splited_n_gram:
                    if splited in self.n_gram_index_map.keys():
                        n_gram_index.append(self.n_gram_index_map[splited])
...
def test_WordHash():
    ...
    n_gram_matrix = wordhash.get_n_gram_count(['cc', 'cd','key words based'])

    #print(n_gram_matrix[0].toarray())
    #print(cosine_similarity(n_gram_matrix[0].toarray().reshape(1,-1), n_gram_matrix[1].toarray().reshape(1,-1)))
